(function(b) {
    function a(a, c, b) {
        ...
        m = 0; m < d; m++) {
            ...
            for (var n = 1; n < d; n++) o = a[n].x * l.x + a[n].y * l.y,
            o > k ? k = o: o < j && (j = o);
            p = q = c[0].x * l.x + c[0].y * l.y;
            for (n = 1; n < i; n++) o = c[n].x * l.x + c[n].y * l.y,
            o > q ? q = o: o < p && (p = o);
            j < p ? (j = p - k, l.x = -l.x, l.y = -l.y) : j -= q;
            if (j >= 0) return ! 1;
            else if (j > b.overlap) b.overlap = j,
            b.normal.x = l.x,
            b.normal.y = l.y
        }
        return b
    ...
    c.merge = function(a, c, b) {
        for (var d in c) if (!b || a.hasOwnProperty(d) || a[d] !== void 0) a[d] = c[d];
        return a
    };
    ...
    c.getElementOffset = function(a) {
        ...
        b = a.offsetTop; (a = a.offsetParent) && a != document.body && a != document;) c += a.offsetLeft,
        b += a.offsetTop;
        return {
            left: c,
            top: b
    ...
    c.createDOMDrawable = function(a, b) {
        ...
        k.style.width = i + "px";
        k.style.height = j + "px";
        if (d == "canvas") k.width = i,
        k.height = j,
        g && (d = k.getContext("2d"), i = b.rect || [0, 0, i, j], d.drawImage(g, i[0], i[1], i[2], i[3], a.x || 0, a.y || 0, a.width || i[2], a.height || i[3]));
        else if (k.style.opacity = a.alpha != void 0 ? a.alpha: 1, k.style.overflow = "hidden", g && g.src) k.style.backgroundImage = "url(" + g.src + ")",
        k.style.backgroundPosition = -(a.rectX || 0) + "px " + -(a.rectY || 0) + "px";
        return k
    };
    ...
    b.prototype.unregister = function(a, c, b) {
        for (var e = 0; e < c.length; e++) for (var f = c[e], h = this._evtHandlers[f], g = 0; g < h.length; g++) {
            var i = h[g];
    ...
    b.prototype._onEvent = function(a, c, d) {
        var e = a,
        f = a.type;
        if (a.type.indexOf("touch") == 0) e = a.touches && a.touches.length > 0 ? a.touches[0] : a.changedTouches && a.changedTouches.length > 0 ? a.changedTouches[0] : a,
        e.type = f;
    ...
    b.prototype.removeEventListener = function(a, c) {
        var b = this._eventMap[a];
        if (b == null) return ! 1;
        for (var e = 0; e < b.length; e++) if (b[e] === c) return b.splice(e, 1),
        b.length == 0 && delete this._eventMap[a],
        !0;
        return ! 1
    };
    ...
    function b(a, c) {
        for (var d = 0; d < a.children.length; d++) {
            ...
            if (g.children) b(g, c);
                ...
                c.stroke();
                c.closePath()
            } else if (g.drawable.domDrawable) g.drawable.domDrawable.style.border = "1px solid #f00"
        }
    }
    Quark.getUrlParams = function() {
        ...
        b = c.indexOf("?");
        if (b > 0) for (var c = c.substring(b + 1).split("&"), b = 0, d; d = c[b]; b++) e = c[b] = d.split("="),
        d = e,
    ...
    b.prototype._run = function() {
        if (!this.paused) {
            ...
            f, h; b < e; b++) f = this._listeners[b],
            h = f.__runTime || 0,
            h == 0 ? f.step(this.info) : c > h && (f.step(this.info), this._listeners.splice(b, 1), b--, e--);
            a.lastTime = c
        }
    };
    ...
    var b = Quark.Tween = function(c, b, e) {
        ...
        for (var f in b) {
            var h = c[f],
            g = b[f];
            h !== void 0 && typeof h == "number" && typeof g == "number" && (this._oldProps[f] = h, this._newProps[f] = g, this._deltaProps[f] = g - h)
        }
        for (f in e) this[f] = e[f]
    };
    b.prototype.setProps = function(a, b) {
        for (var e in a) this.target[e] = this._oldProps[e] = a[e];
        for (e in b) this._newProps[e] = b[e],
    ...
    b.prototype._update = function() {
        if (!this.paused) {
            ...
            if (! (d < 0)) {
                ...
                if (a >= 1) {
                    if (this.reverse) {
                        if (e = this._oldProps, this._oldProps = this._newProps, this._newProps = e, this._startTime = Date.now(), this._frameCount = 0, this._reverseFlag *= -1, !this.loop) this.reverse = !1
                    } else if (this.loop) {
                        for (e in this._oldProps) this.target[e] = this._oldProps[e];
                        this._startTime = Date.now();
                        this._frameCount = 0
                    } else if (b.remove(this), e = this.next, e != null && (e instanceof b ? (a = e, e = null) : a = e.shift(), a != null)) a.next = e,
                    a.start();
    ...
    b._tweens = [];
    b.step = function() {
        for (var a = this._tweens,
        b = a.length; --b >= 0;) a[b]._update()
    };
    b.add = function(a) {
    ...
    a.Elastic.EaseIn = function(a) {
        ...
        else f || (f = 0.3); ! e || e < 1 ? (e = 1, b = f / 4) : b = f / (2 * Math.PI) * Math.asin(1 / e);
    ...
    b.prototype._evtHandler = function(a) {
        if (a.type == "progress") {
            ...
            d = 0,
            e = a.target.buffered;
            if (e && e.length > 0) for (b = e.length; b >= 0; b--) d = e.end(b) - e.start(b);
            if (d / a.target.duration >= 1) this._element.removeEventListener("progress", this._evtHandler),
        ...
        } else a.type == "ended" ? (this.dispatchEvent({
            type: "ended",
            target: this
    ...
    b.prototype.set = function(a, b) {
        ...
        else if (this.domDrawable) this.domDrawable.style.backgroundImage = "url(" + this.rawDrawable.src + ")"
    }
})(); (function() {
    ...
    b.prototype._render = function(a) {
        a = this.context || a; ! this.visible || this.alpha <= 0 ? (a.hide != null && a.hide(this), this.saveState(["visible", "alpha"])) : (a.startDraw(), a.transform(this), this.render(a), a.endDraw(), this.saveState())
    };
    ...
    var b = Quark.DisplayObjectContainer = function(a) {
        ...
        this.children = [];
        if (a.children) for (var c = 0; c < a.children.length; c++) this.addChild(a.children[c])
    };
    ...
    b.prototype.addChild = function(a) {
        ...
        d = 0; d < arguments.length; d++) a = arguments[d],
        this.addChildAt(a, b + d);
        return this
    };
    b.prototype.removeChildAt = function(a) {
        if (a < 0 || a >= this.children.length) return ! 1;
        var b = this.children[a];
        if (b != null) this.getStage().context.remove(b),
        b.parent = null;
    ...
    b.prototype.render = function(a) {
        ...
        for (var c = 0,
        d = this.children.length; c < d; c++) this.children[c]._render(a)
    };
    b.prototype.getObjectUnderPoint = function(a, b, d, e) {
        if (e) var f = [];
        for (var h = this.children.length - 1; h >= 0; h--) {
            var g = this.children[h];
            if (! (g == null || !g.eventEnabled && g.children == void 0 || !g.visible || g.alpha <= 0)) if (g.children != void 0 && g.eventChildren && g.getNumChildren() > 0) {
                var i = g.getObjectUnderPoint(a, b, d, e);
                if (i) if (e) i.length > 0 && (f = f.concat(i));
                else return i;
                else if (g.hitTestPoint(a, b, d) >= 0) if (e) f.push(g);
                else return g
            } else if (g.hitTestPoint(a, b, d) >= 0) if (e) f.push(g);
            else return g
        }
    ...
    b.prototype._onEvent = function(a) {
        ...
        a.eventX = b;
        a.eventY = d;
        if (e != null && e != f) a.lastEventTarget = e,
        (b = a.type == "mousemove" ? "mouseout": a.type == "touchmove" ? "touchout": null) && e._onEvent({
            type: b
        }),
        this._eventTarget = null;
        if (f != null && f.eventEnabled) a.eventTarget = e = this._eventTarget = f,
        f._onEvent(a);
    ...
    b.prototype.addFrame = function(a) {
        ...
        if (a instanceof Array) for (var d = 0; d < a.length; d++) this.setFrame(a[d], b + d);
    ...
    b.prototype.getFrameIndex = function(a) {
        ...
        for (var a = this._frameLabels[a], b = this._frames, d = 0; d < b.length; d++) if (a == b[d]) return d;
        return - 1
    };
    b.prototype.nextFrame = function(a) {
        var b = this._frames[this.currentFrame];
        if (b.interval > 0) a = this._displayedCount + a,
        this._displayedCount = b.interval > a ? a: 0;
        if (b.jump >= 0 || typeof b.jump == "string") if (this._displayedCount == 0 || !b.interval) return this.currentFrame = this.getFrameIndex(b.jump);
    ...
    b.prototype.setEnabled = function(a) {
        if (this.enabled == a) return this; (this.eventEnabled = this.enabled = a) ? this.currentFrame == 3 && this._skin.gotoAndStop(b.UP) : this.disabledState ? this._skin.gotoAndStop(b.DISABLED) : this._skin.gotoAndStop(b.state.UP);
        return this
    };
    ...
    b.prototype.transform = function(a) {
        var b = this.context; (a.x != 0 || a.y != 0) && b.translate(a.x, a.y);
        a.rotation % 360 != 0 && b.rotate(a.rotation % 360 * Quark.DEG_TO_RAD); (a.scaleX != 1 || a.scaleY != 1) && b.scale(a.scaleX, a.scaleY); (a.regX != 0 || a.regY != 0) && b.translate( - a.regX, -a.regY);
    ...
    h.prototype.transform = function(a) {
        ...
        if (a.transformEnabled || !a._addedToDOM) {
            ...
            if (!c.style.backgroundPosition || a.propChanged("rectX", "rectY")) c.style.backgroundPosition = -a.rectX + "px " + -a.rectY + "px";
            if (!c.style.width || a.propChanged("width", "height")) c.style.width = a.width + "px",
            c.style.height = a.height + "px";
            if (!c.style[e] || a.propChanged("regX", "regY")) c.style[e] = a.regX + "px " + a.regY + "px";
            if (!c.style[d] || a.propChanged("x", "y", "regX", "regY", "scaleX", "scaleY", "rotation")) e = Quark.supportTransform3D ? b(a, !0) : b(a, !1),
            c.style[d] = e;
